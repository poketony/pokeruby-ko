...
const u8 BattleText_GainExpPoints[] = _("{STRING 0}{STRING 1}{STRING 42} 경험치를 얻었다!\p"); //JP: 3DA4C6
const u8 BattleText_Terminator[] = _("\s\n");
const u8 BattleText_BoostedExp[] = _("\s 많은 양의\n");
const u8 BattleText_GrewLevel[] = _("{STRING 0}\s\n레벨{STRING 1}\d로 올랐다!{UNKNOWN_A}\p");
const u8 BattleText_LearnedMove[] = _("{STRING 0}\s\n{STRING 1}\f 배웠다!{UNKNOWN_A}\p");
const u8 BattleText_TryingToLearnMove[] = _("{STRING 0}\s 새로\n{STRING 1}\f 배우고 싶다...\p");
const u8 BattleText_CantLearnMore[] = _("그러나 {STRING 0}\s 기술을 4개\n알고 있으므로 더 이상 배울 수 없다!\p");
const u8 BattleText_DeleteMove[] = _("{STRING 1} 대신\n다른 기술을 잊게 하겠습니까?");
const u8 BattleText_DeletedMove[] = _("{STRING 0}\s {STRING 1}\f\n깨끗이 잊었다!\p");
const u8 BattleText_StopLearning[] = _("{PAUSE 32}그럼... {STRING 1}\f\n배우는 것을 포기하겠습니까?");
const u8 BattleText_DidNotLearn[] = _("{STRING 0}\s {STRING 1}\f\n결국 배우지 않았다!\p");
const u8 BattleText_UseNext[] = _("다음 포켓몬을 사용하겠습니까?");
const u8 BattleText_AttackMissed[] = _("그러나 {ATTACKING_MON}의\n공격은 빗나갔다!");
const u8 BattleText_ProtectedItself[] = _("{DEFENDING_MON}\s\n공격으로부터 몸을 지켰다!");
const u8 BattleText_AvoidedDamage[] = _("{DEFENDING_MON}\s {STRING 22} 때문에\n데미지를 입지 않는다!");
const u8 BattleText_GroundMoveNegate[] = _("{DEFENDING_MON}\s {STRING 22} 때문에\n땅타입 기술에 당하지 않는다!");
const u8 BattleText_AvoidedAttack[] = _("{DEFENDING_MON}에게는\n맞지 않았다!");
const u8 BattleText_DoesntAffect[] = _("{DEFENDING_MON}에게는\n효과가 없는 것 같다...");
const u8 BattleText_AttackingFainted[] = _("{ATTACKING_MON}\s 쓰러졌다!\p");
const u8 BattleText_DefendingFainted[] = _("{DEFENDING_MON}\s 쓰러졌다!\p");
const u8 BattleText_WinningPrize[] = _("{STRING 32}\s 상금으로\n{STRING 0}원을 손에 넣었다!\p");
const u8 BattleText_OutOfUsablePoke[] = _("{STRING 32}에게는\n싸울 수 있는 포켓몬이 없다!\p");
const u8 BattleText_WhitedOut[] = _("{STRING 32}\s\n눈앞이 캄캄해졌다!{PAUSE_UNTIL_PRESS}");
const u8 BattleText_PreventEscape[] = _("{STRING 16}의\n{STRING 23} 때문에 도망칠 수 없다!\p");
const u8 BattleText_CantEscape[] = _("도망칠 수 없다!\p");
const u8 BattleText_AttackingCantEscape[] = _("{ATTACKING_MON}\s\n도망칠 수 없다!");
const u8 BattleText_HitMulti[] = _("{STRING 0}번 맞았다!");
const u8 BattleText_FellAsleep[] = _("{STRING 14}\s\n잠들어 버렸다!");
const u8 BattleText_MadeAsleep[] = _("{STRING 16}의 {STRING 23} 때문에\n{STRING 14}\s 잠들어 버렸다!");
const u8 BattleText_DefendingAsleep[] = _("{DEFENDING_MON}\s\n이미 잠들어 있다");
const u8 BattleText_AttackingAsleep[] = _("{ATTACKING_MON}\s\n이미 잠들어 있다");
...
const u8 BattleText_PoisonHurt[] = _("{ATTACKING_MON}\s\n독에 의한 데미지를 입었다!");
const u8 BattleText_AlreadyPoisoned[] = _("{DEFENDING_MON}\s 이미\n몸에 독이 퍼진 상태다");
const u8 BattleText_BadlyPoisoned[] = _("{STRING 14}의\n몸에 맹독이 퍼졌다!");
const u8 BattleText_EnergyDrained[] = _("{DEFENDING_MON}\d로부터\n체력을 흡수했다!");
const u8 BattleText_Burned[] = _("{STRING 14}\s\n화상을 입었다!");
const u8 BattleText_Burned2[] = _("{STRING 16}의 {STRING 23} 때문에\n{STRING 14}\s 화상을 입었다!");
const u8 BattleText_BurnHurt[] = _("{ATTACKING_MON}\s\n화상 데미지를 입었다!");
const u8 BattleText_AlreadyBurned[] = _("{DEFENDING_MON}\s 이미\n화상을 입은 상태다");
const u8 BattleText_Frozen[] = _("{STRING 14}\s\n얼어붙었다!");
const u8 BattleText_Frozen2[] = _("{STRING 16}의 {STRING 23} 때문에\n{STRING 14}\s 얼어붙었다!");
const u8 BattleText_FrozenSolid[] = _("{ATTACKING_MON}\s\n얼어버려서 움직일 수 없다!");
...
const u8 BattleText_Paralyzed[] = _("{STRING 14}\s 마비되어\n기술이 나오기 어려워졌다!");
const u8 BattleText_Paralyzed2[] = _("{STRING 16}의 {STRING 23} 때문에\n{STRING 14}\s 마비되어!\l기술이 나오기 어려워졌다!");
const u8 BattleText_Paralyzed3[] = _("{ATTACKING_MON}\s\n몸이 저려서 움직일 수 없다!");
const u8 BattleText_AlreadyParalyzed[] = _("{DEFENDING_MON}\s\n이미 마비되어 있다");
...
const u8 BattleText_StoppedWorking[] = _("우리 편 {STRING 0}\r\n멈췄다!");
const u8 BattleText_StoppedWorking2[] = _("상대 {STRING 0}\r\n멈췄다!");
const u8 BattleText_Confused[] = _("{ATTACKING_MON}\s\n혼란에 빠져 있다!");
const u8 BattleText_ConfusionSnapOut[] = _("{ATTACKING_MON}의\n혼란이 풀렸다!");
const u8 BattleText_BecameConfused[] = _("{STRING 14}\s\n혼란에 빠졌다!");
const u8 BattleText_AlreadyConfused[] = _("{DEFENDING_MON}\s 이미\n혼란에 빠져 있다");
const u8 BattleText_FellLove[] = _("{DEFENDING_MON}\s\n헤롱헤롱해졌다!");
const u8 BattleText_InLoveWith[] = _("{ATTACKING_MON}\s\n{STRING 16}에게 헤롱헤롱해 있다!");
const u8 BattleText_ImmobilizedBy[] = _("{ATTACKING_MON}\s\n헤롱헤롱해서 기술을 쓸 수 없었다!");
const u8 BattleText_BlownAway[] = _("{DEFENDING_MON}\s\n날라갔다!");
const u8 BattleText_TypeTransform[] = _("{ATTACKING_MON}\s\n{STRING 0}타입이 됐다!");
const u8 BattleText_Flinched[] = _("{ATTACKING_MON}\s\n풀이 죽어 기술을 쓸 수 없다!");
const u8 BattleText_RegainedHealth[] = _("{DEFENDING_MON}\s체력을 회복했다!");
...
const u8 BattleText_CoveredVeil[] = _("{STRING 38}\s\n신비의 베일에 둘러싸였다!");
const u8 BattleText_SafeguardActive[] = _("{DEFENDING_MON}\f\n신비의 베일이 지켜주고 있다!");
const u8 BattleText_SafeguardFaded[] = _("{STRING 40}\f 감싸던\n신비의 베일이 없어졌다!");
const u8 BattleText_WentToSleep[] = _("{ATTACKING_MON}\s\n잠을 자기 시작했다!");
const u8 BattleText_SpeltHealthy[] = _("{ATTACKING_MON}\s\n잠이 들어 건강해졌다!");
const u8 BattleText_WhipWhirlwind[] = _("{ATTACKING_MON}의 주위에서\n공기가 소용돌이친다!");
const u8 BattleText_TookSunlight[] = _("{ATTACKING_MON}\s\n빛을 흡수했다!");
const u8 BattleText_LoweredHead[] = _("{ATTACKING_MON}\s\n목을 움츠렸다!");
const u8 BattleText_IsGlowing[] = _("{ATTACKING_MON}\f\n세찬 빛이 감쌌다!");
const u8 BattleText_FlewHigh[] = _("{ATTACKING_MON}\s\n하늘 높이 날아올랐다!");
const u8 BattleText_DugHole[] = _("{ATTACKING_MON}\s\n땅으로 파고들었다!");
const u8 BattleText_HidUnderwater[] = _("{ATTACKING_MON}\s\n물속에 몸을 숨겼다!");
const u8 BattleText_SprangUp[] = _("{ATTACKING_MON}\s\n높이 뛰어올랐다!");
const u8 BattleText_SqueezedBind[] = _("{DEFENDING_MON}\s\n{ATTACKING_MON}에게 조이기를 당했다!");
const u8 BattleText_TrappedVortex[] = _("{DEFENDING_MON}\s\n소용돌이 속에 갇혔다!");
const u8 BattleText_SandTombTrapped[] = _("{DEFENDING_MON}\s\n모래지옥에 붙잡혔다!");
const u8 BattleText_Wrapped[] = _("{DEFENDING_MON}\s\n{ATTACKING_MON}에게 휘감겼다!");
const u8 BattleText_Clamped[] = _("{ATTACKING_MON}\s\n{DEFENDING_MON}의 껍질에 꼈다!");
const u8 BattleText_HurtBy[] = _("{ATTACKING_MON}\s {STRING 0}의\n데미지를 입고 있다");
const u8 BattleText_FreedFrom[] = _("{ATTACKING_MON}\s\n{STRING 0}\d로부터 풀려났다!");
const u8 BattleText_KeptGoingCrash[] = _("{ATTACKING_MON}\s\n의욕이 넘쳐서 땅에 부딪혔다!");
const u8 BattleText_MistShroud[] = _("{STRING 38}\n흰안개에 둘러싸였다!");
const u8 BattleText_MistProtect[] = _("{STRING 16}\f\n흰안개가 지켜주고 있다!");
const u8 BattleText_GetPumped[] = _("{ATTACKING_MON}\s\n힘이 넘쳐흐른다!");
const u8 BattleText_HitRecoil[] = _("{ATTACKING_MON}\s\n반동으로 데미지를 입었다!");
const u8 BattleText_ProtectedItself2[] = _("{ATTACKING_MON}\s\n방어 태세에 들어갔다!");
...
const u8 BattleText_EvadedAttack[] = _("{DEFENDING_MON}\s\n공격을 피했다!");
const u8 BattleText_HealthSapped[] = _("씨뿌리기가 {ATTACKING_MON}의\n체력을 빼앗는다!");
const u8 BattleText_FastAsleep[] = _("{ATTACKING_MON}\s\n쿨쿨 잠들어 있다");
const u8 BattleText_WokeUp[] = _("{ATTACKING_MON}\s\n눈을 떴다!");
const u8 BattleText_UproarAwake[] = _("그러나 {STRING 16}\s\n소란스러워서 잠들 수 없다!");
const u8 BattleText_UproarWoke[] = _("{ATTACKING_MON}\s\n소란스러워서 눈을 떴다!");
const u8 BattleText_UproarCaused[] = _("{ATTACKING_MON}\s\n소란을 피우기 시작했다!");
const u8 BattleText_UproarMaking[] = _("{ATTACKING_MON}\s\n소란을 피우고 있다!");
const u8 BattleText_CalmedDown[] = _("{ATTACKING_MON}\s\n얌전해졌다!");
const u8 BattleText_UproarCantSleep[] = _("그러나 {DEFENDING_MON}\s\n소란스러워서 잠들 수 없다!");
const u8 BattleText_Stockpiled[] = _("{ATTACKING_MON}\s\n{STRING 0}개 비축했다!");
const u8 BattleText_StockpiledCant[] = _("{ATTACKING_MON}\s\n더 이상 비축할 수 었다!");
const u8 BattleText_UproarCantSleep2[] = _("그러나 {DEFENDING_MON}\s\n소란을 피우고 있어서 잠들지 않는다!");
const u8 BattleText_UproarKeptAwake[] = _("{DEFENDING_MON}\s\n소란스러워서 눈을 떴다!");
const u8 BattleText_StayedAwake[] = _("{DEFENDING_MON}\s\n{STRING 22} 때문에 잠들지 않는다!");
const u8 BattleText_StoringEnergy[] = _("{ATTACKING_MON}\s 참고 있다");
const u8 BattleText_UnleashedEnergy[] = _("{ATTACKING_MON}의\n참기가 풀렸다!");
const u8 BattleText_FatigueConfuse[] = _("{ATTACKING_MON}\s\n몹시 지쳐서 혼란에 빠졌다!");
const u8 BattleText_PickedUpYen[] = _("{STRING 32}\s {STRING 0}원을 주웠다!\p");
const u8 BattleText_Unaffected[] = _("{DEFENDING_MON}에게는\n전혀 효과가 없다!");
const u8 BattleText_Transformed[] = _("{ATTACKING_MON}\s\n{STRING 0}\d로 변신했다!");
const u8 BattleText_SubMade[] = _("{ATTACKING_MON}의\n대타가 나타났다!");
const u8 BattleText_SubAlready[] = _("{ATTACKING_MON}의\n대타는 이미 나와 있다");
const u8 BattleText_SubTookDamage[] = _("{DEFENDING_MON}\f 대신하여\n대타가 공격을 받았다!\p");
const u8 BattleText_SubFaded[] = _("{DEFENDING_MON}의\n대타는 사라져 버렸다...\p");
const u8 BattleText_MustRecharge[] = _("{ATTACKING_MON}\s\n공격의 반동으로 움직일 수 없다!");
const u8 BattleText_RageBuilding[] = _("{DEFENDING_MON}의\n분노 볼티지가 올라가고 있다!");
const u8 BattleText_MoveWasDisabled[] = _("{DEFENDING_MON}의\n{STRING 0}\f 봉인했다!");
const u8 BattleText_DisabledNoMore[] = _("{ATTACKING_MON}의\n사슬묶기가 풀렸다!");
const u8 BattleText_EncoreGot[] = _("{DEFENDING_MON}\s\n앵콜을 받았다!");
const u8 BattleText_EncoreEnded[] = _("{ATTACKING_MON}의\n앵콜 상태가 풀렸다!");
const u8 BattleText_TookAim[] = _("{ATTACKING_MON}\s\n{DEFENDING_MON}에게 조준했다!");
const u8 BattleText_SketchedMove[] = _("{ATTACKING_MON}\s\n{STRING 0}\f 스케치했다!");
const u8 BattleText_DestinyBondTake[] = _("{ATTACKING_MON}\s 상대를\n길동무로 삼으려 하고 있다!");
const u8 BattleText_DestinyBondTaken[] = _("{DEFENDING_MON}\s {ATTACKING_MON}\f\n길동무로 삼았다!");
const u8 BattleText_ReducedBy[] = _("{DEFENDING_MON}의\n{STRING 0}\f {STRING 1} 깎았다!");
const u8 BattleText_StoleSomething[] = _("{ATTACKING_MON}\s\n{DEFENDING_MON}의 {STRING 19}\f 빼앗았다!");
const u8 BattleText_CantEscapeNow[] = _("{DEFENDING_MON}\s\n이제 도망칠 수 없다!");
const u8 BattleText_NightmareStart[] = _("{DEFENDING_MON}\s\n악몽을 꾸기 시작했다!");
const u8 BattleText_NightmareLock[] = _("{ATTACKING_MON}\s\n악몽에 시달리고 있다!");
const u8 BattleText_CurseLay[] = _("{ATTACKING_MON}\s\n자신의 체력을 깎아서\l{DEFENDING_MON}에게 저주를 걸었다!");
const u8 BattleText_CurseAfflict[] = _("{ATTACKING_MON}\s\n저주받고 있다!");
const u8 BattleText_SpikesScattered[] = _("{STRING 37}의 발밑에\n압정이 뿌려졌다!");
const u8 BattleText_SpikesHurt[] = _("{STRING 16}\s\n압정뿌리기의 데미지를 입었다!");
const u8 BattleText_IdentifiedPoke[] = _("{ATTACKING_MON}\s\n{DEFENDING_MON}\f 간파했다!");
const u8 BattleText_PerishSongFell[] = _("{ATTACKING_MON}의\n멸망의 카운트가 {STRING 0} 되었다!");
const u8 BattleText_BracedItself[] = _("{ATTACKING_MON}\s\n버티기 태세에 들어갔다!");
const u8 BattleText_EnduredHit[] = _("{DEFENDING_MON}\s\n공격을 버텼다!");
const u8 BattleText_MagnitudeCount[] = _("매그니튜드 {STRING 0}!");
const u8 BattleText_CutHPMaxATK[] = _("{ATTACKING_MON}\s\n체력을 깎아서 풀 파워로 만들었다!");
const u8 BattleText_CopyStatChanges[] = _("{ATTACKING_MON}\s\n{DEFENDING_MON}의\n능력 변화를 복사했다!");
const u8 BattleText_GotFreeFrom[] = _("{ATTACKING_MON}\s\n{DEFENDING_MON}의 {STRING 0}\d로부터 풀려났다!");
const u8 BattleText_LeechShed[] = _("{ATTACKING_MON}에게\n씨앗을 심었다!");
const u8 BattleText_SpikesBlownAway[] = _("{ATTACKING_MON}의\n압정이 사라졌다!");
const u8 BattleText_FledBattle[] = _("{ATTACKING_MON}\s\n배틀에서 이탈했다!");
const u8 BattleText_ForesawAttack[] = _("{ATTACKING_MON}\s\n미래의 공격을 예지했다!");
const u8 BattleText_TookAttack[] = _("{DEFENDING_MON}\s\n{STRING 0} 공격을 받았다!");
const u8 BattleText_ChoseDestiny[] = _("{ATTACKING_MON}\s\n{STRING 17}\f 미래에 맡겼다!");
const u8 BattleText_PokeAttack[] = _("{STRING 0}의 공격!");
const u8 BattleText_CenterAttention[] = _("{ATTACKING_MON}\s\n주목의 대상이 되었다!");
const u8 BattleText_ChargingPower[] = _("{ATTACKING_MON}\s\n충전을 시작했다!");
const u8 BattleText_NaturePower[] = _("자연의힘은\n{STRING 17}\r 되었다!!");
const u8 BattleText_StatusNormal[] = _("{ATTACKING_MON}의\n상태이상을 회복했다!");
const u8 BattleText_TormentSubject[] = _("{DEFENDING_MON}\s\n트집을 잡혔다!");
const u8 BattleText_TightenFocus[] = _("{ATTACKING_MON}\s\n집중력을 높이고 있다!");
const u8 BattleText_TauntFell[] = _("{DEFENDING_MON}\s\n도발에 넘어가 버렸다!");
const u8 BattleText_ReadyToHelp[] = _("{ATTACKING_MON}\s {DEFENDING_MON}에게\n도우미가 되어주려 한다!");
const u8 BattleText_SwitchedItems[] = _("{ATTACKING_MON}\s 서로의\n도구를 교체했다!");
const u8 BattleText_Obtained1[] = _("{ATTACKING_MON}\s\n{STRING 0}\f 손에 넣었다!");
const u8 BattleText_Obtained2[] = _("{DEFENDING_MON}\s\n{STRING 1}\f 손에 넣었다!");
const u8 BattleText_Obtained3[] = _("{ATTACKING_MON}\s\n{STRING 0}\f 손에 넣었다!\p{DEFENDING_MON}\s\n{STRING 1}\f 손에 넣었다!");
const u8 BattleText_CopiedObject[] = _("{ATTACKING_MON}\s\n{DEFENDING_MON}의\l{STRING 22}\f 복사했다!");
const u8 BattleText_WishMade[] = _("{ATTACKING_MON}\s\n신비한 달빛에 둘러싸였다!");
const u8 BattleText_WishTrue[] = _("{STRING 0}의\n희망사항이 이루어졌다!");
const u8 BattleText_PlantedRoots[] = _("{ATTACKING_MON}\s 뿌리를 뻗었다!");
const u8 BattleText_AbsorbNutrients[] = _("{ATTACKING_MON}\s 뿌리로부터\n양분을 흡수했다!");
const u8 BattleText_AnchoredItself[] = _("{DEFENDING_MON}\s 뿌리를 뻗어서\n움직이지 않는다!");
const u8 BattleText_DrowsyMade[] = _("{ATTACKING_MON}\s {DEFENDING_MON}의\n졸음을 유도했다!");
const u8 BattleText_KnockedOffItem[] = _("{ATTACKING_MON}\s\n{DEFENDING_MON}의\l {STRING 19}\f 탁 쳐서 떨구었다!");
const u8 BattleText_AbilitySwap[] = _("{ATTACKING_MON}\s\n서로의 특성을 교체했다!");
const u8 BattleText_SealedMove[] = _("{ATTACKING_MON}\s\n상대의 기술을 봉인했다!");
const u8 BattleText_GrudgeBear[] = _("{ATTACKING_MON}\s 상대에게\n원념을 걸려 하고 있다!");
const u8 BattleText_GrudgeLosePP[] = _("{ATTACKING_MON}의 {STRING 0}\s\n원념으로 PP가 0이 되었다!");
const u8 BattleText_ShroudedItself[] = _("{ATTACKING_MON}\s\n{STRING 17}에 둘러싸였다!");
const u8 BattleText_MagicCoatBounce[] = _("{ATTACKING_MON}의 {STRING 17}\s\n매직코트로 되받아쳤다!");
const u8 BattleText_AwaitMove[] = _("{ATTACKING_MON}\s\n상대의 움직임을 살피고 있다!");
const u8 BattleText_SnatchedMove[] = _("{DEFENDING_MON}\s\n{STRING 16}의\n기술을 가로챘다!");
const u8 BattleText_ElecWeakened[] = _("전기의 위력이 약해졌다!");
const u8 BattleText_FireWeakened[] = _("불꽃의 위력이 약해졌다!");
const u8 BattleText_FoundOne[] = _("{ATTACKING_MON}\s\n {STRING 19}\f 주웠다!");
const u8 BattleText_SoothingAroma[] = _("기분 좋은 향기가 퍼졌다!");
const u8 BattleText_CantUseItems[] = _("여기서는 도구를 쓸 수 없습니다...{PAUSE 64}");
const u8 BattleText_UnknownString2[] = _("{STRING 16}\s {STRING 19}에게는\n{STRING 0}");
const u8 BattleText_HustleUse[] = _("{STRING 16}\s\n{STRING 19}\f 써서 의욕이 넘치기 시작했다!");
const u8 BattleText_LostFocus[] = _("{ATTACKING_MON}\s 집중력이\n떨어져서 기술을 쓸 수 없다!");
const u8 BattleText_DraggedOut[] = _("{DEFENDING_MON}\s\n배틀에 끌려 나왔다!\p");
const u8 BattleText_BrokeWall[] = _("벽이 무너졌다!");
const u8 BattleText_NoEffect[] = _("그러나 효과가 없었다!");
const u8 BattleText_NoMovesLeft[] = _("{STRING 15}\s\n쓸 수 있는 기술이 없다!\p");
const u8 BattleText_MoveIsDisabled[] = _("{STRING 15}\s 사슬묶기 때문에\n{STRING 17}\f 쓸 수 없다!\p");
const u8 BattleText_TormentNoUse[] = _("{STRING 15}\s\n트집을 잡혔기 때문에\r\n계속해서 같은 기술을 쓸 수 없다!\p");
const u8 BattleText_TauntNoUse[] = _("{STRING 15}\s 도발당한 상태라서\n{STRING 17}\f 쓸 수 없다!\p");
const u8 BattleText_SealedNoUse[] = _("{STRING 15}\s 봉인 때문에\n{STRING 17}\f 쓸 수 없다!\p");
...
const u8 BattleText_PreventedBy[] = _("{DEFENDING_MON}의 {STRING 22} 때문에\n{ATTACKING_MON}\s\l{STRING 17}\f 쓸 수 없다!");
const u8 BattleText_HPRestoredUsing[] = _("{DEFENDING_MON}의\n{STRING 22}\d로 체력을 회복했다!");
...
const u8 BattleText_PreventedPara[] = _("{STRING 14}의 {STRING 22} 때문에\n마비되지 않는다!");
const u8 BattleText_PreventedRomance[] = _("{DEFENDING_MON}의 {STRING 22} 때문에\n헤롱헤롱되지 않는다!");
const u8 BattleText_PreventedPoison[] = _("{STRING 14}의 {STRING 22} 때문에\n독에 중독되지 않는다!");
const u8 BattleText_PreventedConfusion[] = _("{DEFENDING_MON}의 {STRING 22} 때문에\n혼란되지 않는다!");
const u8 BattleText_RaisedFirePower[] = _("{DEFENDING_MON}의 {STRING 22} 때문에\n불꽃의 위력이 올라갔다!");
const u8 BattleText_AnchorsItself[] = _("{DEFENDING_MON}의 {STRING 22} 때문에\n엉겨 붙었다!");
const u8 BattleText_CutsAttack[] = _("{STRING 16}의 {STRING 23} 때문에\n{DEFENDING_MON}의\l공격이 떨어졌다!");
const u8 BattleText_PreventedStatLoss[] = _("{STRING 16}의 {STRING 23} 때문에\n능력이 떨어지지 않는다!");
const u8 BattleText_HurtOther[] = _("{DEFENDING_MON}의 {STRING 22} 때문에\n{ATTACKING_MON}\s\l상처를 입었다!");
const u8 BattleText_Traced[] = _("{STRING 16}\s {STRING 0}의\n{STRING 1}\f 트레이스했다!");
const u8 BattleText_PreventedBurn[] = _("{STRING 14}의 {STRING 24} 때문에\n화상을 입지 않는다!");
const u8 BattleText_BlocksOther[] = _("{DEFENDING_MON}의 {STRING 22} 때문에\n{STRING 17}\f 받지 않는다!");
const u8 BattleText_BlocksOther2[] = _("{STRING 16}의 {STRING 23} 때문에\n{STRING 17}\f 받지 않는다!");
const u8 BattleText_RestoredHPByItem[] = _("{ATTACKING_MON}\s {STRING 21}\d로 인해 조금 회복했다");
...
const u8 BattleText_PreventedLoss[] = _("{STRING 16}의 {STRING 23} 때문에\n{STRING 0}\r 떨어지지 않는다!");
const u8 BattleText_InfatuatedPoke[] = _("{DEFENDING_MON}의 {STRING 22} 때문에\n{ATTACKING_MON}\s 헤롱헤롱해졌다!");
const u8 BattleText_MadeIneffective[] = _("{DEFENDING_MON}의 {STRING 22} 때문에\n{STRING 17}\s 효과가 없다!");
const u8 BattleText_CuredProblem[] = _("{STRING 16}의 {STRING 23} 때문에\n{STRING 0}상태가 나았다!");
const u8 BattleText_OozeSuckup[] = _("해감액을 흡수했다!");
const u8 BattleText_Transformed2[] = _("{STRING 16}\s\n다른 모습으로 변화했다");
const u8 BattleText_TookAttack2[] = _("{DEFENDING_MON}의 {STRING 22} 때문에\n공격을 받았다!");
const u8 BattleText_PreventedSwitch[] = _("{STRING 0}의 {STRING 20} 때문에\n교체할 수 없다!\p");
const u8 BattleText_PreventedOther[] = _("{DEFENDING_MON}의 {STRING 22} 때문에\n{STRING 16}의\l{STRING 0}\s 효과가 없었다!");
...
const u8 BattleText_PreventedOther2[] = _("{ATTACKING_MON}의 {STRING 21} 때문에\n{DEFENDING_MON}의\l{STRING 22}\s 효과가 없었다!");
const u8 BattleText_CuredOfProblem[] = _("{STRING 16}의 {STRING 23} 때문에\n{STRING 0}상태가 나았다!");
const u8 BattleText_NoEffectOn[] = _("{STRING 16}의 {STRING 23}\s\n{STRING 14}에게는 효과가 없었다!");
...
const u8 BattleText_UnknownString4[] = _("{STRING 19}\d로 {STRING 16}의\n{STRING 0}\r {STRING 1}");
const u8 BattleText_UnknownString5[] = _("{ATTACKING_MON}의\n{STRING 0}\r {STRING 1}");
const u8 BattleText_UnknownString6[] = _("{DEFENDING_MON}의\n{STRING 0}\r {STRING 1}");
const u8 BattleText_StatNoHigher[] = _("{ATTACKING_MON}의\n{STRING 0}\s 더 올라가지 않는다!");
const u8 BattleText_StatNoLower[] = _("{DEFENDING_MON}의\n{STRING 0}\s 더 떨어지지 않는다!");
const u8 BattleText_Critical[] = _("급소에 맞았다!");
const u8 BattleText_GrandSlam[] = _("일격필살!");
const u8 BattleText_MoveForget123[] = _("{PAUSE 32}1, {PAUSE 15}2 ...{PAUSE 15} ...{PAUSE 15}\n{PAUSE 15}{PLAY_SE SE_KON}짠!\p");
...
const u8 BattleText_FledUsingItem[] = _("{PLAY_SE SE_NIGERU}{ATTACKING_MON}\s 가지고 있던\n{STRING 19}\f 써서 도망쳤다!\p");
const u8 BattleText_FledUsingOther[] = _("{PLAY_SE SE_NIGERU}{ATTACKING_MON}\s 가지고 있던\n{STRING 21}\f 써서 도망쳤다!\p");
const u8 BattleText_FledWild[] = _("{PLAY_SE SE_NIGERU}야생 {STRING 0}\s\n도망쳤다!");
const u8 BattleText_PlayerDefeatedTrainer[] = _("{STRING 29}\h의\n승부에서 이겼다!");
const u8 BattleText_PlayerDefeatedTrainers[] = _("{STRING 30}&{STRING 29}\h의\n승부에서 이겼다!");
const u8 BattleText_PlayerLostTrainer[] = _("{STRING 29}\h의\n승부에서 졌다!");
const u8 BattleText_PlayerLostTrainers[] = _("{STRING 30}&{STRING 29}\h의\n승부에서 졌다!");
const u8 BattleText_PlayerTiedTrainer[] = _("{STRING 29}\h의\n승부에서 비겼다!");
const u8 BattleText_PlayerTiedTrainers[] = _("{STRING 30}&{STRING 29}\h의\n승부에서 비겼다!");
const u8 BattleText_FledSingle[] = _("{PLAY_SE SE_NIGERU}{STRING 29}\s 도망쳤다!");
const u8 BattleText_FledDouble[] = _("{PLAY_SE SE_NIGERU}{STRING 29}\h {STRING 30}\s 도망쳤다!");
...
const u8 BattleText_SandRages[] = _("모래바람이 계속 불고있다");
...
const u8 BattleText_SunFaded[] = _("햇살이 원래대로 되돌아왔다!");
const u8 BattleText_HailStart[] = _("싸라기눈이 내리기 시작했다!");
const u8 BattleText_HailCont[] = _("싸라기눈이 계속 휘몰아친다");
...
const u8 BattleText_StatElim[] = _("모든 능력변화가\n원래대로 되돌아왔다!");
const u8 BattleText_CoinScatter[] = _("돈이 주위에 흩어졌다!");
const u8 BattleText_SubWeak[] = _("그러나 대타를 내보내기에는\n체력이 부족했다!");
...
const u8 BattleText_NoPP2[] = _("그러나 남은 기술 포인트가\n없었다!");
const u8 BattleText_IgnoredOrdersSLP[] = _("{ATTACKING_MON}\s 잠든 채로\n명령을 무시했다!");
const u8 BattleText_IgnoredOrders[] = _("{ATTACKING_MON}\s 명령을 무시했다!");
const u8 BattleText_BeganNap[] = _("{ATTACKING_MON}\s 낮잠을 자기 시작했다!");
const u8 BattleText_LoafingAround[] = _("{ATTACKING_MON}\s 게으름을 피우고 있다!");
const u8 BattleText_WontObey[] = _("{ATTACKING_MON}\s 말을 듣지 않는다!");
const u8 BattleText_TurnedAway[] = _("{ATTACKING_MON}\s 외면했다!");
const u8 BattleText_NotNotice[] = _("{ATTACKING_MON}\s 모른 체했다!");
const u8 BattleText_WillSwitch[] = _("{STRING 25} {STRING 26}\s\n{STRING 1}\f 내보내려 하고 있다\l포켓몬을 교체하시겠습니까?");
const u8 BattleText_LearnedMove2[] = _("{ATTACKING_MON}\s\n{STRING 0}\f 배웠다!");
const u8 BattleText_PlayerDefeatedTrainer2[] = _("{STRING 25} {STRING 26}\h의\n승부에서 이겼다!\p");
const u8 BattleText_CreptCloser[] = _("{STRING 32}\s\n{STRING 3}에게 가까이 다가갔다!");
const u8 BattleText_CantGetCloser[] = _("{STRING 32}\s\n더 이상 다가갈 수 없다!");
const u8 BattleText_WatchingCarefully[] = _("{STRING 3}\s\n상황을 살피고 있다!");
const u8 BattleText_CuriousAbout[] = _("{STRING 3}\s\n{STRING 0}에 대해 관심을 가졌다!");
const u8 BattleText_EnthralledBy[] = _("{STRING 3}\s\n{STRING 0}\r 마음에 들었다!");
const u8 BattleText_IgnoredThing[] = _("{STRING 3}\s\n{STRING 0}\f 무시했다!");
const u8 BattleText_ThrewBlock[] = _("{STRING 32}\s\n{STRING 3}에게 먹이를 던졌다!");
const u8 BattleText_SafariOver[] = _("{PLAY_SE SE_PINPON}아나운서: 딩동-!\n사파리볼이 전부 떨어졌으므로\l종료입니다-!\p");
const u8 BattleText_WildAppeared1[] = _("앗! 야생 {STRING 3}\r 튀어나왔다!\p");
const u8 BattleText_WildAppeared2[] = _("앗! 야생 {STRING 3}\r 튀어나왔다!\p");
const u8 BattleText_WildAppeared3[] = _("앗! 야생 {STRING 3}\r 튀어나왔다!{PAUSE 127}");
const u8 BattleText_WildDoubleAppeared[] = _("앗! 야생\n{STRING 5}\h {STRING 3}\r 튀어나왔다!\p");
const u8 BattleText_SingleWantToBattle1[] = _("{STRING 25} {STRING 26}\r\n승부를 걸어왔다!\p");
const u8 BattleText_SingleWantToBattle2[] = _("{STRING 29}\r\n승부를 걸어왔다!");
const u8 BattleText_DoubleWantToBattle[] = _("{STRING 29}\h {STRING 30}\r\n승부를 걸어왔다!");
const u8 BattleText_SentOutSingle1[] = _("{STRING 25} {STRING 26}\s\n{STRING 3}\f 내보냈다!");
const u8 BattleText_SentOutDouble1[] = _("{STRING 25} {STRING 26}\s\n{STRING 3}\h {STRING 5}\f 내보냈다!");
const u8 BattleText_SentOutSingle2[] = _("{STRING 25} {STRING 26}\s\n{STRING 0}\f 내보냈다!");
const u8 BattleText_SentOutSingle3[] = _("{STRING 29}\s\n{STRING 3}\f 내보냈다!");
const u8 BattleText_SentOutDouble2[] = _("{STRING 29}\s\n{STRING 3}\h {STRING 5}\f 내보냈다!");
const u8 BattleText_SentOutDouble3[] = _("{STRING 29}\s\n{STRING 7}\f 내보냈다!\p{STRING 30}\s\n{STRING 9}\f 내보냈다!");
const u8 BattleText_SentOutSingle4[] = _("{STRING 29}\s\n{STRING 0}\f 내보냈다!");
const u8 BattleText_SentOutSingle5[] = _("{STRING 31}\s\n{STRING 0}\f 내보냈다!");
...
const u8 BattleText_SentOutSingle11[] = _("{STRING 28}\s {STRING 8}\f 꺼냈다!\n가랏! {STRING 6}!");
...
const u8 BattleText_WithdrewPoke1[] = _("{STRING 25} {STRING 26}\s\n{STRING 0}\f 넣어버렸다!");
const u8 BattleText_WithdrewPoke2[] = _("{STRING 29}\s\n{STRING 0}\f 넣어버렸다!");
const u8 BattleText_WithdrewPoke3[] = _("{STRING 31}\s\n{STRING 0}\f 넣어버렸다!");
...
const u8 BattleText_OpponentUsedMove[] = _("{ATTACKING_MON}\s\n{STRING 1}");
const u8 BattleText_Exclamation2[] = _("\f 썼다!");
const u8 BattleText_Exclamation3[] = _("\f 썼다!");
...
const u8 BattleText_Used1[] = _("{STRING 32}\s\n{STRING 19}\f 썼다!");
const u8 BattleText_TutorialUsed[] = _("민진은\n{STRING 19}\f 썼다!");
const u8 BattleText_Used2[] = _("{STRING 25} {STRING 26}\s\n{STRING 19}\f 썼다!");
...
const u8 BattleText_BallBrokeOhNo[] = _("안 돼! 포켓몬이\n볼에서 나와버렸다!");
...
const u8 BattleText_BallCaught1[] = _("신난다-!\n{STRING 3}\f 붙잡았다!{UNKNOWN_A}{PLAY_BGM BGM_KACHI22}\p");
const u8 BattleText_BallCaught2[] = _("신난다-!\n{STRING 3}\f 붙잡았다!{UNKNOWN_A}{PLAY_BGM BGM_KACHI22}{PAUSE 127}");
const u8 BattleText_GiveNickname[] = _("잡은 {STRING 3}에게\n닉네임을 붙이겠습니까?");
const u8 BattleText_SentToPC[] = _("{STRING 3}\s\n{STRING 35} PC로 전송되었다!");
...
const u8 BattleText_Sandstorm[] = _("모래바람이 계속 불고있다");
const u8 BattleText_BoxFull[] = _("박스가 가득 찼습니다!\n더 이상 잡을 수 없습니다!\p");
const u8 BattleText_EnigmaBerry[] = _("의문열매");
const u8 BattleText_Berry[] = _("열매");
const u8 BattleText_CuredParalysis[] = _("{STRING 16}\s {STRING 19}\d로\n마비가 풀렸다!");
const u8 BattleText_CuredPoison[] = _("{STRING 16}\s {STRING 19}\d로\n독이 해독됐다!");
const u8 BattleText_CuredBurn[] = _("{STRING 16}\s {STRING 19}\d로\n화상이 나았다!");
const u8 BattleText_CuredFreeze[] = _("{STRING 16}\s {STRING 19}\d로\n얼음 상태가 나았다!");
const u8 BattleText_CuredSleep[] = _("{STRING 16}\s {STRING 19}\d로\n눈을 떴다!");
const u8 BattleText_CuredConfusion[] = _("{STRING 16}\s {STRING 19}\d로\n혼란이 풀렸다!");
const u8 BattleText_CuredStatus[] = _("{STRING 16}\s {STRING 19}\d로\n{STRING 0}상태가 나았다!");
const u8 BattleText_NormalizedStatus[] = _("{STRING 16}\s {STRING 19}\d로\n상태이상이 회복되었다!");
const u8 BattleText_RestoredHealth[] = _("{STRING 16}\s {STRING 19}\d로\n체력을 회복했다!");
const u8 BattleText_RestoredPP[] = _("{STRING 16}\s {STRING 19}\d로\n{STRING 0}의 PP를 회복했다!");
const u8 BattleText_RestoredStatus[] = _("{STRING 16}\s {STRING 19}\d로\n상태이상을 회복했다!");
const u8 BattleText_RestoredHPLittle[] = _("{STRING 16}\s\n{STRING 19}\d로 인해 조금 회복했다!");
const u8 BattleText_ChoiceBand[] = _("{STRING 19}의 효과로\n{STRING 17}밖에 쓸 수 없다!\p");
const u8 BattleText_FocusSash[] = _("{DEFENDING_MON}\s\n{STRING 19}\d로 버텼다!");
const u8 BattleText_Terminator2[] = _("");
const u8 BattleText_WallyBall[] = _("지금 볼을 던지면 되는 건가요?\n더... 던질게요!");
const u8 BattleText_StartEvo[] = _("...오잉!?\n{STRING 2}의 모습이...!");
const u8 BattleText_FinishEvo[] = _("축하합니다! {STRING 2}\s\n{STRING 3}\d로 진화했습니다!{UNKNOWN_A}\p");
const u8 BattleText_StopEvo[] = _("얼라리...?\n{STRING 2}의 변화가 멈췄다!\p");
const u8 BattleText_OtherMenu[] = _("{STRING 15}\s\n무엇을 할까?");
const u8 BattleText_PlayerMenu[] = _("{STRING 32}\s\n무엇을 할까?");
...
const u8 BattleText_Format10[] = _("\s");
