...
static EWRAM_DATA f32 gLogoX = 0;
static EWRAM_DATA f32 gLogoY = 0;
...
void SpriteCallback_VersionBannerLeft(struct Sprite *sprite)
{
    ...
    if (task->data[1] != 0)
    {
		...
		sprite->pos1.x = VERSION_BANNER_LEFT_X;
        sprite->pos1.y = VERSION_BANNER_Y;
    ...
    else
    {
        ...
        {
            sprite->invisible = FALSE;
            if (sprite->pos1.x != VERSION_BANNER_LEFT_X)
                sprite->pos1.x += 2;
            sprite->pos1.y = VERSION_BANNER_Y;
...
void SpriteCallback_VersionBannerRight(struct Sprite *sprite)
{
    ...
    {
        sprite->oam.objMode = 0;
		sprite->pos1.x = VERSION_BANNER_RIGHT_X;
        sprite->pos1.y = VERSION_BANNER_Y;
        sprite->invisible = FALSE;
    }
    else
    {
        ...
        {
            sprite->invisible = FALSE;
            if (sprite->pos1.x != VERSION_BANNER_RIGHT_X)
                sprite->pos1.x += 2;
            sprite->pos1.y = VERSION_BANNER_Y;
        }
    }
...
static void Task_TitleScreenPhase2(u8 taskId)
{
	...
	{
		//로고 이동
		if (!(gTasks[taskId].tCounter & 1) && gLogoX < 0)
			gLogoX += 0.875;
		else if (gLogoX >= 0)
			gLogoX = 0;
		if (!(gTasks[taskId].tCounter & 1) && gLogoY < -4)
			gLogoY += 0.875;
		else if (gLogoY >= -4)
			gLogoY = -4;
