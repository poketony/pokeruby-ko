...
# 6 "src/field/bike.c" 2
# 1 "include/flags.h" 1
# 94 "include/flags.h"
bool8 FlagGet(u16 flag);
# 7 "src/field/bike.c" 2
# 1 "include/global.fieldmap.h" 1
# 8 "src/field/bike.c" 2
...
static const struct BikeHistoryInputInfo gAcroBikeTricksList[] =
{
    ...
    {DIR_SOUTH, 0x0002, 0xF, 0xF, AcroBikeJumpTimerList, AcroBikeJumpTimerList, DIR_SOUTH},
    {DIR_NORTH, 0x0002, 0xF, 0xF, AcroBikeJumpTimerList, AcroBikeJumpTimerList, DIR_NORTH},
    {DIR_WEST, 0x0002, 0xF, 0xF, AcroBikeJumpTimerList, AcroBikeJumpTimerList, DIR_WEST},
    {DIR_EAST, 0x0002, 0xF, 0xF, AcroBikeJumpTimerList, AcroBikeJumpTimerList, DIR_EAST},
};
...
static void AcroBikeTransition_SideJump(u8 direction)
{
    ...
    if (var != 0)
    {
        ...
        if (WillPlayerCollideWithCollision(var, direction) == 0)
        {
            AcroBikeTransition_TurnDirection(direction);
            return;
        }
...
bool8 IsRunningDisallowed(u8 tile)
{
    if (IsRunningDisallowedByMetatile(tile) != 0 || gMapHeader.mapType == MAP_TYPE_INDOOR)
        return 1;
    else
        return 0;
}
...
bool8 player_should_look_direction_be_enforced_upon_movement(void)
{
    if (TestPlayerAvatarFlags((1 << 2)) != 0 && MetatileBehavior_IsBumpySlope(gMapObjects[gPlayerAvatar.mapObjectId].mapobj_unk_1E) != 0)
        return 0;
    else
        return 1;
}
