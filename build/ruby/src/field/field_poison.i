...
# 2 "src/field/field_poison.c" 2
# 1 "include/field_poison.h" 1



s32 DoPoisonFieldEffect(void);
# 3 "src/field/field_poison.c" 2
...
# 5 "src/field/field_poison.c" 2
# 1 "include/pokemon.h" 1
# 6 "src/field/field_poison.c" 2
...
bool32 CheckMonIsValid(struct Pokemon *pkmn)
{
    ...
    u16 species2 = GetMonData(pkmn, 65);
...
bool32 AllMonsFainted(void)
{
    ...
    for (i = 0; i < 6; i++, pkmn++)
    {
        ...
        if (CheckMonIsValid(pkmn) && GetMonData(pkmn, 57) != 0)
            return 0;
    }
    return 1;
}
...
void MonFaintFromPoisonOnField(u8 partyMember)
{
    ...
    SetMonData(pkmn, 55, (u8*)&val);
    GetMonData(pkmn, 2, gStringVar1);
...
bool32 CheckMonFaintedFromPoison(u8 partyMember)
{
    ...
    if (CheckMonIsValid(pkmn) && GetMonData(pkmn, 57) == 0
     && pokemon_ailments_get_primary(GetMonData(pkmn, 55)) == 1)
        return 1;
    else
        return 0;
}
...
s32 DoPoisonFieldEffect(void)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        if (GetMonData(pkmn, 5) != 0
         && pokemon_ailments_get_primary(GetMonData(pkmn, 55)) == 1)
        {
            hp = GetMonData(pkmn, 57);
            if (hp != 0)
                hp--;
            if (hp == 0)
                numFainting++;
            SetMonData(pkmn, 57, &hp);
            numPoisoned++;
        }
        pkmn++;
    }
