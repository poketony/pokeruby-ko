...
# 6 "src/pokemon/pokemon_3.c" 2
# 1 "include/hold_effects.h" 1
# 7 "src/pokemon/pokemon_3.c" 2
# 1 "include/item.h" 1
...
# 12 "src/pokemon/pokemon_3.c" 2
# 1 "include/pokemon.h" 1
# 13 "src/pokemon/pokemon_3.c" 2
# 1 "include/rng.h" 1
...
# 21 "src/pokemon/pokemon_3.c" 2
# 1 "include/sprite.h" 1
# 22 "src/pokemon/pokemon_3.c" 2
...
# 25 "src/pokemon/pokemon_3.c" 2
# 1 "include/ewram.h" 1



extern u8 gSharedMem[];
# 26 "src/pokemon/pokemon_3.c" 2
...
{
    u32 status = GetMonData(mon, 55, 0);

    if (status & healMask)
    {
        status &= ~healMask;
        SetMonData(mon, 55, &status);
...
u16 GetEvolutionTargetSpecies(struct Pokemon *mon, u8 type, u16 evolutionItem)
{
    int i;
    u16 targetSpecies = 0;
    u16 species = GetMonData(mon, 11, 0);
    u16 heldItem = GetMonData(mon, 12, 0);
    u32 personality = GetMonData(mon, 0, 0);
    u8 level;
    u16 friendship;
    u8 beauty = GetMonData(mon, 23, 0);
    ...
    {
    case 0:
        level = GetMonData(mon, 56, 0);
        friendship = GetMonData(mon, 32, 0);
        ...
        for (i = 0; i < 5; i++)
        {
            ...
            case 0x0008:
                if (gEvolutionTable[species].evolutions[i].param <= level)
                    if (GetMonData(mon, 59, 0) > GetMonData(mon, 60, 0))
                        targetSpecies = gEvolutionTable[species].evolutions[i].targetSpecies;
                break;
            case 0x0009:
                if (gEvolutionTable[species].evolutions[i].param <= level)
    ...
    case 3:
        for (i = 0; i < 5; i++)
        {
            if (gEvolutionTable[species].evolutions[i].method == 0x0007
...
{
    u16 species = GetMonData(mon, 65, 0);
    u16 heldItem = GetMonData(mon, 12, 0);
    u8 holdEffect;
    ...
    if (species && species != SPECIES_EGG)
    {
        ...
        s16 friendship = GetMonData(mon, 32, 0);
        if (friendship > 99)
            friendshipLevel++;
        if (friendship > 199)
            friendshipLevel++;
            ...
            || gTrainers[gTrainerBattleOpponent].trainerClass == 32))))
        {
            ...
            if (mod > 0)
            {
                if (GetMonData(mon, 38, 0) == ITEM_LUXURY_BALL)
                    friendship++;
                if (GetMonData(mon, 35, 0) == sav1_map_get_name())
                    friendship++;
            }
            if (friendship < 0)
                friendship = 0;
...
void RandomlyGivePartyPokerus(struct Pokemon *party)
{
    ...
    if (rnd == 0x4000 || rnd == 0x8000 || rnd == 0xC000)
    {
        ...
        do
        {
            ...
            while (!GetMonData(mon, 11, 0));
        }
        while (GetMonData(mon, 45, 0));
...
u8 CheckPartyPokerus(struct Pokemon *party, u8 selection)
{
    ...
    if (selection)
    {
        do
        {
            if ((selection & 1) && (GetMonData(&party[partyIndex], 34, 0) & 0xF))
                retVal |= curBit;
            partyIndex++;
            curBit <<= 1;
            selection >>= 1;
    ...
    else if (GetMonData(&party[0], 34, 0) & 0xF)
    {
        retVal = 1;
    }
...
u8 CheckPartyHasHadPokerus(struct Pokemon *party, u8 selection)
{
    ...
    if (selection)
    {
        do
        {
            if ((selection & 1) && GetMonData(&party[partyIndex], 34, 0))
                retVal |= curBit;
            partyIndex++;
            curBit <<= 1;
            selection >>= 1;
    ...
    else if (GetMonData(&party[0], 34, 0))
    {
        retVal = 1;
    }
...
void UpdatePartyPokerusTime(u16 days)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        {
            u8 pokerus = GetMonData(&gPlayerParty[i], 34, 0);
            if (pokerus & 0xF)
            {
                ...
                SetMonData(&gPlayerParty[i], 34, &pokerus);
            }
        }
    }
}
...
void PartySpreadPokerus(struct Pokemon *party)
{
    if ((Random() % 3) == 0)
    {
        ...
        for (i = 0; i < 6; i++)
        {
            ...
            {
                u8 pokerus = GetMonData(&party[i], 34, 0);
                ...
                if (pokerus)
                {
                    if (pokerus & 0xF)
                    {
                        ...
                        if (i != 0 && !(GetMonData(&party[i - 1], 34, 0) & 0xF0))
                            SetMonData(&party[i - 1], 34, &curPokerus);
                        if (i != (6 - 1) && !(GetMonData(&party[i + 1], 34, 0) & 0xF0))
                        {
                            SetMonData(&party[i + 1], 34, &curPokerus);
                            i++;
                        }
...
{
    u16 species = GetMonData(mon, 11, 0);
    u8 nextLevel = GetMonData(mon, 56, 0) + 1;
    if (GetMonData(mon, 25, 0) > gExperienceTables[gBaseStats[species].growthRate][nextLevel])
    {
        SetMonData(mon, 56, &nextLevel);
        return 1;
    }
    else
    {
...
u8 GetMoveRelearnerMoves(struct Pokemon *mon, u16 *moves)
{
    ...
    u8 numMoves = 0;
    u16 species = GetMonData(mon, 11, 0);
    u8 level = GetMonData(mon, 56, 0);
    int i, j, k;

    for (i = 0; i < 4; i++)
        learnedMoves[i] = GetMonData(mon, 13 + i, 0);
...
bool8 IsTradedMon(struct Pokemon *mon)
{
    u8 otName[8];
    u32 otId;
    GetMonData(mon, 7, otName);
    otId = GetMonData(mon, 1, 0);
...
void BoxMonRestorePP(struct BoxPokemon *boxMon)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (GetBoxMonData(boxMon, 13 + i, 0))
        {
            u16 move = GetBoxMonData(boxMon, 13 + i, 0);
            u16 bonus = GetBoxMonData(boxMon, 21, 0);
            u8 pp = CalculatePPWithBonus(move, bonus, i);
            SetBoxMonData(boxMon, 17 + i, &pp);
        }
    }
}
...
void sub_8040B8C(void)
{
    gLastUsedAbility = ((struct BattleStruct *) (gSharedMem + 0x0))->unk160C0;;
    gBattleTextBuff1[0] = 0xFD;
    gBattleTextBuff1[1] = 4;
    gBattleTextBuff1[2] = ((struct BattleStruct *) (gSharedMem + 0x0))->unk16054;
    gBattleTextBuff1[4] = 0xFF;
    if (!GetBankSide(((struct BattleStruct *) (gSharedMem + 0x0))->unk16054))
        gBattleTextBuff1[3] = pokemon_order_func(gBattlePartyID[((struct BattleStruct *) (gSharedMem + 0x0))->unk16054]);
    else
        gBattleTextBuff1[3] = gBattlePartyID[((struct BattleStruct *) (gSharedMem + 0x0))->unk16054];
...
void SetWildMonHeldItem(void)
{
    if (!(gBattleTypeFlags & (0x2000 | 0x0008)))
    {
        u16 rnd = Random() % 100;
        u16 species = GetMonData(&gEnemyParty[0], 11, 0);
        if (gBaseStats[species].item1 == gBaseStats[species].item2)
        {
            SetMonData(&gEnemyParty[0], 12, &gBaseStats[species].item1);
            return;
        }
        ...
        if (rnd > 44)
        {
            if (rnd <= 94)
                ...
                SetMonData(&gEnemyParty[0], 12, &gBaseStats[species].item2);
        }
    }
}
...
{
    bool8 retVal = 0;
    u32 shinyValue = (((otId) & 0xFFFF0000) >> 16) ^ ((otId) & 0xFFFF) ^ (((personality) & 0xFFFF0000) >> 16) ^ ((personality) & 0xFFFF);
    if (shinyValue < 8)
        retVal = 1;
    return retVal;
}
...
bool32 unref_sub_8040DAC(struct Pokemon *mon)
{
    u8 name[12];
    u16 species = GetMonData(mon, 11, 0);
    u8 language = GetMonData(mon, 3, 0);
