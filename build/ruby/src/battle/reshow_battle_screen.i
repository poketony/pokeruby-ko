# 1 "src/battle/reshow_battle_screen.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "src/battle/reshow_battle_screen.c"
...
# 2 "src/battle/reshow_battle_screen.c" 2
...
# 3 "src/battle/reshow_battle_screen.c" 2
...
# 4 "src/battle/reshow_battle_screen.c" 2
...
# 5 "src/battle/reshow_battle_screen.c" 2
# 1 "include/main.h" 1
...
# 6 "src/battle/reshow_battle_screen.c" 2
...
# 7 "src/battle/reshow_battle_screen.c" 2
# 1 "include/text.h" 1
# 77 "include/text.h"
struct WindowConfig
{
...
# 8 "src/battle/reshow_battle_screen.c" 2
...
# 9 "src/battle/reshow_battle_screen.c" 2
# 1 "include/data2.h" 1
...
# 10 "src/battle/reshow_battle_screen.c" 2
# 1 "include/ewram.h" 1



extern u8 gSharedMem[];
# 11 "src/battle/reshow_battle_screen.c" 2
...
static void CB2_ReshowBattleScreenAfterMenu(void)
{
    ...
    case 1:
        {
            const u32 zero = 0;
            CpuFastSet(&zero, (void*) 0x6000000, 0x1006000);
        }
        break;
    case 2:
        if (!sub_800E414((gSharedMem[0x1FFFE])))
        {
            (gSharedMem[0x1FFFE])++;
            (gSharedMem[0x1FFFF])--;
        }
    ...
    case 5:
        ...
        break;
    case 6:
        if (sub_8031C30((gSharedMem[0x1FFFE])))
            (gSharedMem[0x1FFFE]) = 0;
        else
        {
            (gSharedMem[0x1FFFE])++;
    ...
    case 19:
        {
            ...
            species = GetMonData(&gEnemyParty[gBattlePartyID[opponentBank]], 11);
...
static void sub_807B06C(void)
{
    ...
    regBgcnt1 = (void*)(&(*(vu16 *)(0x4000000 + 0xa)));
    regBgcnt1->charBaseBlock = 0;

    regBgcnt2 = (void*)(&(*(vu16 *)(0x4000000 + 0xc)));
...
static bool8 LoadAppropiateBankSprite(u8 bank)
{
    if (bank < gNoOfAllBanks)
    {
        if (GetBankSide(bank))
        {
            if (!((struct Struct2017800 *) (gSharedMem + 0x17800))[bank].substituteSprite)
                BattleLoadOpponentMonSprite(&gEnemyParty[gBattlePartyID[bank]], bank);
            else
                BattleLoadSubstituteSprite(bank, 0);
        }
        else if (gBattleTypeFlags & 0x0080 && bank == 0)
            LoadPlayerTrainerBankSprite(gSaveBlock2.playerGender, 0);
        else if (gBattleTypeFlags & 0x0200 && bank == 0)
            LoadPlayerTrainerBankSprite(2, 0);
        else if (!((struct Struct2017800 *) (gSharedMem + 0x17800))[bank].substituteSprite)
...
static void sub_807B184(u8 bank)
{
    if (bank < gNoOfAllBanks)
    {
        ...
        if (GetBankSide(bank))
        {
            if (GetMonData(&gEnemyParty[gBattlePartyID[bank]], 57) == 0)
                return;
            GetMonSpriteTemplate_803C56C(GetMonData(&gEnemyParty[gBattlePartyID[bank]], 11), GetBankIdentity(bank));
            ...
            gSprites[gObjectBankIDs[bank]].data[2] = GetMonData(&gEnemyParty[gBattlePartyID[bank]], 11);
        ...
        {
            if (GetMonData(&gPlayerParty[gBattlePartyID[bank]], 57) == 0)
                return;
            GetMonSpriteTemplate_803C56C(GetMonData(&gPlayerParty[gBattlePartyID[bank]], 11), GetBankIdentity(bank));
            gObjectBankIDs[bank] = CreateSprite(&gUnknown_02024E8C, sub_8077ABC(bank, 2), posY, sub_8079E90(bank));
            gSprites[gObjectBankIDs[bank]].oam.paletteNum = bank;
            gSprites[gObjectBankIDs[bank]].callback = SpriteCallbackDummy;
            gSprites[gObjectBankIDs[bank]].data[0] = bank;
            gSprites[gObjectBankIDs[bank]].data[2] = GetMonData(&gPlayerParty[gBattlePartyID[bank]], 11);
            StartSpriteAnim(&gSprites[gObjectBankIDs[bank]], gBattleMonForms[bank]);
        }
        gSprites[gObjectBankIDs[bank]].invisible = ((struct Struct2017800 *) (gSharedMem + 0x17800))[bank].invisible;
    }
}
...
static void sub_807B508(u8 bank)
{
    ...
    {
        u8 healthboxID;
        if (gBattleTypeFlags & 0x0080 && bank == 0)
