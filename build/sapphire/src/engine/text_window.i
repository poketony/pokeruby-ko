...
# 5 "src/engine/text_window.c" 2
# 1 "include/text.h" 1
# 6 "src/engine/text_window.c" 2
...
void LoadTextWindowGraphics(struct Window *win)
{
    u8 *tileData = win->config->tileData + 32 * sTextWindowBaseTileNum;
...
static void LoadTextWindowTiles(u8 frameType, void *dest)
{
    ...
    CpuFastSet(frameGraphics->tiles, dest, ((9 * 32)/(32/8) & 0x1FFFFF));
}
...
static void DrawMessageBox(struct Window *win, u8 left, u8 top, u8 width, u8 height)
{
    ...
    for (i = 0; i < height + 2; i++)
        for (j = 0; j < width + 4; j++)
            tilemap[(left + j + 1) + 32 * (top + i)] = (win->paletteNum << 12) | GetMessageBoxTilemapEntry(tilemapEntry, j, i, width, height);
}

void DrawStandardMessageBox(struct Window *win)
{
    DrawMessageBox(win, 0, 14, 24, 4);
}

void LoadMessageBoxTiles(struct Window *win)
{
    u8 *tileData = win->config->tileData;
    CpuFastSet(gMessageBox_Gfx, tileData + 32 * sMessageBoxBaseTileNum, ((14 * 32)/(32/8) & 0x1FFFFF));
}

void ClearStandardMessageBox(struct Window *win)
{
    u8 i;
    u16 *tilemap = win->config->tilemap + (14 * 32);
