...
# 4 "src/engine/menu.c" 2
# 1 "include/map_obj_lock.h" 1
...
# 10 "src/engine/menu.c" 2
# 1 "include/text.h" 1
# 11 "src/engine/menu.c" 2
...
__attribute__((section("ewram_data"))) struct Window gMenuWindow = {0};
__attribute__((section("ewram_data"))) u8 gFiller_202E908[0x90] = {0};
__attribute__((section("ewram_data"))) struct Window *gMenuWindowPtr = ((void *)0);
__attribute__((section("ewram_data"))) u8 gMenuMultistepInitState = 0;
__attribute__((section("ewram_data"))) u16 gMenuTextTileOffset = 0;
__attribute__((section("ewram_data"))) u16 gMenuTextWindowTileOffset = 0;
__attribute__((section("ewram_data"))) u16 gMenuTextWindowContentTileOffset = 0;
__attribute__((section("ewram_data"))) u16 gMenuMessageBoxContentTileOffset = 0;
...
{
    sub_8002EB0(gMenuWindowPtr, str, gMenuTextTileOffset, 4, 15);
}
...
u8 MoveMenuCursor(s8 delta)
{
 ...
 MenuPrint(space, gMenu.left + 1, 2 * gMenu.cursorPos + gMenu.top);
...
u8 MoveMenuCursor3(s8 delta)
{
 ...
 MenuPrint(space, 6 * (gMenu.cursorPos / menuHeight) + gMenu.left + 1,
...
u8 MoveMenuCursor4(s8 delta)
{
 ...
 {
  MenuPrint(space, 1 + gMenu.left + gMenu.columnXCoords[gMenu.cursorPos % gMenu.width],
...
static bool8 sub_80723D4(void)
{
    if ((gMain.newKeys & 0x0040) && gMenu.cursorPos < gMenu.width)
        return 1;

    if ((gMain.newKeys & 0x0080) && gMenu.cursorPos >= (gMenu.maxCursorPos + 1) - gMenu.width)
        return 1;

    if ((gMain.newKeys & 0x0020)
     && ((gMenu.cursorPos - (gMenu.cursorPos % gMenu.width)) % gMenu.width == 1
         || gMenu.cursorPos == 0
         || gMenu.cursorPos % gMenu.width == 0))
         return 1;

    if ((gMain.newKeys & 0x0010) && gMenu.cursorPos % gMenu.width == gMenu.width - 1)
        return 1;
...
static u8 sub_80724F4(u8 left, u8 top, u8 menuItemCount, const struct MenuAction menuItems[], u8 columnCount)
{
    ...
    {
        u8 width = (sub_8072CA4(menuItems[i].text) + 7) / 8 + 1;
...
static void sub_8072620(u8 left, u8 top, u8 menuItemCount, const struct MenuAction menuItems[], u8 columnCount)
{
    ...
    for (i = 0; i < columnCount; i++)
    {
        ...
        for (j = 0; i + j < menuItemCount; j += columnCount, row++)
            MenuPrint(menuItems[i + j].text, 1 + left + gMenu.columnXCoords[i % columnCount], top + 2 * row);
    }
}
...
void PrintMenuItems(u8 left, u8 top, u8 menuItemCount, const struct MenuAction menuItems[])
{
    ...
    for (i = 0; i < menuItemCount; i++)
        MenuPrint(menuItems[i].text, left + 1, top + 2 * i);
}
...
void PrintMenuItemsReordered(u8 left, u8 top, u8 menuItemCount, const struct MenuAction2 menuItems[], const u8 *order)
{
    ...
    for (i = 0; i < menuItemCount; i++)
        MenuPrint(menuItems[order[i]].text, left + 1, top + 2 * i);
}
...
{
    u8 newlineCount = sub_8004FD0(gMenuWindowPtr, ((void *)0), str, gMenuTextTileOffset, left, top, width, a6);
