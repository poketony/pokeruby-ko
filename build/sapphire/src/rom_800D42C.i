...
# 5 "src/rom_800D42C.c" 2
# 1 "include/ewram.h" 1



extern u8 gSharedMem[];
# 6 "src/rom_800D42C.c" 2
...
# 32 "src/rom_800D42C.c"
void PrintLinkBattleWinLossTie(void)
{

    if (gBattleOutcome == 3)
    {
        { InitWindow(&gUnknown_03004210, BattleText_Tie, 160, 13, 2); sub_8002F44(&gUnknown_03004210); };
        return;
    }
    ...
    if (gBattleTypeFlags & 0x0040)
    {
        ...
        if (gBattleOutcome == 1)
        {
            ...
            switch (gLinkPlayers[((struct BattleStruct *) (gSharedMem + 0x0))->linkPlayerIndex].lp_field_18)
            {
            case 0:
            case 2:
                { InitWindow(&gUnknown_03004210, BattleText_Win, 160, 6, 2); sub_8002F44(&gUnknown_03004210); };
                { InitWindow(&gUnknown_03004210, BattleText_Loss, 168, 21, 2); sub_8002F44(&gUnknown_03004210); };
                return;

            case 1:
            case 3:
                { InitWindow(&gUnknown_03004210, BattleText_Win, 160, 21, 2); sub_8002F44(&gUnknown_03004210); }
                { InitWindow(&gUnknown_03004210, BattleText_Loss, 168, 6, 2); sub_8002F44(&gUnknown_03004210); }
                return;
            }
        }
        else
    ...
    if (gBattleOutcome == 1)
    {
        if (gLinkPlayers[((struct BattleStruct *) (gSharedMem + 0x0))->linkPlayerIndex].lp_field_18 != 0)
        {
            { InitWindow(&gUnknown_03004210, BattleText_Win, 160, 21, 2); sub_8002F44(&gUnknown_03004210); };
            { InitWindow(&gUnknown_03004210, BattleText_Loss, 168, 6, 2); sub_8002F44(&gUnknown_03004210); };
        }
