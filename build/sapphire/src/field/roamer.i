...
# 3 "src/field/roamer.c" 2
# 1 "include/pokemon.h" 1
# 4 "src/field/roamer.c" 2
# 1 "include/rng.h" 1
...
{
    struct Roamer *roamer;
    CreateMon(&gEnemyParty[0], SPECIES_LATIAS, 40, 0x20, 0, 0, 0, 0);
    roamer = &gSaveBlock1.roamer;
    roamer->species = SPECIES_LATIAS;
    roamer->level = 40;
    roamer->status = 0;
    roamer->active = 1;
    roamer->ivs = GetMonData(&gEnemyParty[0], 66);
    roamer->personality = GetMonData(&gEnemyParty[0], 0);
    roamer->hp = GetMonData(&gEnemyParty[0], 58);
    roamer->cool = GetMonData(&gEnemyParty[0], 22);
    roamer->beauty = GetMonData(&gEnemyParty[0], 23);
    roamer->cute = GetMonData(&gEnemyParty[0], 24);
    roamer->smart = GetMonData(&gEnemyParty[0], 33);
    roamer->tough = GetMonData(&gEnemyParty[0], 47);
    sRoamerLocation[MAP_GRP] = 0;
...
void CreateRoamerMonInstance(void)
{
    ...
    SetMonData(mon, 55, &roamer->status);
    SetMonData(mon, 57, &roamer->hp);
    SetMonData(mon, 22, &roamer->cool);
    SetMonData(mon, 23, &roamer->beauty);
    SetMonData(mon, 24, &roamer->cute);
    SetMonData(mon, 33, &roamer->smart);
    SetMonData(mon, 47, &roamer->tough);
}

bool8 TryStartRoamerEncounter(void)
{
    if (IsRoamerAt(gSaveBlock1.location.mapGroup, gSaveBlock1.location.mapNum) == 1 && (Random() % 4) == 0)
    {
