...
# 2 "src/field/lottery_corner.c" 2
...
# 3 "src/field/lottery_corner.c" 2
...
# 4 "src/field/lottery_corner.c" 2
# 1 "include/items.h" 1
...
# 5 "src/field/lottery_corner.c" 2
# 1 "include/rng.h" 1
...
# 6 "src/field/lottery_corner.c" 2
...
# 7 "src/field/lottery_corner.c" 2
...
# 8 "src/field/lottery_corner.c" 2
# 1 "include/text.h" 1
# 77 "include/text.h"
struct WindowConfig
{
...
# 9 "src/field/lottery_corner.c" 2
...
void PickLotteryCornerTicket(void)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        if (GetMonData(pkmn, 11) != SPECIES_NONE)
        {

            if (!GetMonData(pkmn, 45))
            {
                u32 otId = GetMonData(pkmn, 1);
    ...
    for (i = 0; i < 14; i++)
    {
        ...
        for (j = 0; j < 30; j++)
        {
            ...
            if (GetBoxMonData(pkmn, 11) != SPECIES_NONE &&
            !GetBoxMonData(pkmn, 45))
            {
                u32 otId = GetBoxMonData(pkmn, 1);
                u8 numMatchingDigits = GetMatchingDigits(gScriptResult, otId);
    ...
    if (gSpecialVar_0x8004 != 0)
    {
        ...
        else
        {
            ...
            GetBoxMonData(&gPokemonStorage.boxes[box][slot], 2, gStringVar1);
        }
